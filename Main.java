...
public class Main extends Application {
    ...
    public void updateValues() throws IOException, InterruptedException{
            ...
            result_arr[1] = data[0];
            result_arr[0] = data[1];
            raw_adc = ((result_arr[3] & 0xFF) << 24) | ((result_arr[2] & 0xFF) << 16) | ((result_arr[1] & 0xFF) << 8 ) | ((result_arr[0] & 0xFF) << 0 );
            if(raw_adc>32767)
    ...
    public HBox getButtons(){
        ...
        reset_button.addEventFilter(MouseEvent.ANY, new EventHandler<MouseEvent>() {
        ...
        exit_button.setOnAction(e -> ((Stage) exit_button.getScene().getWindow()).close());
        
        box.getChildren().addAll(zero_button, reset_button, shunt_button, exit_button);
    ...
    public VBox getSensitivityLabels(){
        ...
        box.getChildren().addAll(sensitivity_x_title, sensitivity_x_label, sensitivity_y_title, sensitivity_y_label, sensitivity_z_title, sensitivity_z_label);
    ...
    public VBox getShuntEqBox(){
        ...
        TextField shunt_eq_x_field = new TextField(String.format("%1.4f",shunt_eq_x));
        shunt_eq_x_field.textProperty().addListener(new ChangeListener<String>() {
                @Override
        ...
        TextField shunt_eq_y_field = new TextField(String.format("%1.4f",shunt_eq_y));
        shunt_eq_y_field.textProperty().addListener(new ChangeListener<String>() {
                @Override
                public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                        if (!newValue.matches("\\d{0,7}([\\.]\\d{0,4})?")) {
        ...
        TextField shunt_eq_z_field = new TextField(String.format("%1.4f",shunt_eq_z));
        shunt_eq_z_field.textProperty().addListener(new ChangeListener<String>() {
                @Override
                public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                        if (!newValue.matches("\\d{0,7}([\\.]\\d{0,4})?")) {
        ...
        box.getChildren().addAll(shunt_eq_x_label,shunt_eq_x_field,shunt_eq_y_label,shunt_eq_y_field,shunt_eq_z_label,shunt_eq_z_field);
    ...
    public void start(Stage stage) throws FileNotFoundException, IOException, UnsupportedBusNumberException, InterruptedException{
      ...
      led_strip=new DotStar(NUM_LEDS);
        ...
        FileInputStream img = new FileInputStream("/home/pi/Desktop/3DARMDEMO/fair.png");
        root.setBackground(new Background(
                new BackgroundImage(new Image(img), 
        ...
        main_scene.getStylesheets().add(getClass().getResource("armdemo.css").toExternalForm());
        ...
        shunt_scene.getStylesheets().add(getClass().getResource("armdemo.css").toExternalForm());
